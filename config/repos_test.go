...
var _ = Describe("Repository reconfiguration", func() {
	...
	withAuthenticatedAccessToConfigServer := func(tests func()) {
	...
	Describe("Add", func() {
		JustBeforeEach(func() {
			output, err = config.AddGitRepo(fakeCliConnection, fakeAuthClient, configServerSIName, gitURI)
		})

		withAuthenticatedAccessToConfigServer(func() {
			It("should make an authenticated PATCH call with the expected parameters", func() {
				...
				Expect(bodyStr).To(Equal(`{"operation":"add","repo":"my-testuri"}`))
			...
			Context("when the authenticated PATCH call returns an error", func() {
				...
				It("should return the error", func() {
					...
					Expect(err).To(MatchError("Unable to add git repo my-testuri to config server service instance my-configserver: oh dear"))
				})
			})
			...
			Context("when the authenticated PATCH call returns a bad status code", func() {
				...
				It("should return a suitable error", func() {
					Expect(err).To(MatchError("Unable to add git repo my-testuri to config server service instance my-configserver: 404"))
				})
			})
